...
MATERIAL_PATHES = r"K:\Programming\GitHub\tiny-alchemist\scripts\lapgc\working_materials.txt"
...
def main():
    global chat
    materials = [];
    with open(MATERIAL_PATHES, 'r', encoding='utf-8') as f:
        materials = [e[:-1] for e in f.readlines()]
    print(f"\rRelinking {len(materials)} models...                     ");
    
    overrides = [];
    added = 0;
    for i, mat in enumerate(materials):
        overrides.append({ "predicate": { "custom_model_data": i+1  }, "model": f"minecraft:item/{mat}" })
