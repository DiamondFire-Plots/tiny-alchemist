    ...
    if (int(Position.z) == actionBarZ) { //action bar
        if (Color.y == 0) {
            if (int(Color.z*255.0) >= 1) {
                ...
                int yFromBottom = int(bottomRightPosition.y) - int(Position.y);
                if (yFromBottom != overlayTopY && yFromBottom != overlayBottomY) {
                    yFromBottom -= 1; //precision error fix
                }
                if (yFromBottom == overlayTopY || yFromBottom == overlayBottomY) { // main thing
                    vec3 centerPosition = (inverse(ModelViewMat) * inverse(ProjMat) * vec4(0, 0, 0, 1)).xyz;
                    bool xLeft = (int(Position.x) - int(centerPosition.x)) <= 4;
                    float setX = 0;
                    if (xLeft) {
                        setX = 0;
                    } else {
                    ...
                    float setY = 0;
                    if (yFromBottom == overlayTopY) {
                        setY = 0;
                    } else {
                        setY = bottomRightPosition.y;
                    }
                    float ticks = Color.r * 255.0;
                    float g = mod(GameTime * 24000.0, 30.0) / 10.0;
                    float t = clamp(-1 + g, 0, 1);
                    if (ticks <= 0) {
                        t = 0;
                    }
                    if (ticks > 30) {
                    ...
                    color = vec4(0.05, 0.05, 0.05, t*0.75);
                } else { //discard
                    color.w = 0;
                }
            } else { //discard too
